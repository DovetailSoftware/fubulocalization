...
task :compile => [:clean, :version, :restore] do
  MSBuildRunner.compile :compilemode => COMPILE_TARGET, :solutionfile => 'src/FubuLocalization.sln', :clrversion => CLR_VERSION, :extraSwitches => ["p:WarningLevel=1"], :msbuildToolsVersion => MSBUILD_TOOLSVERSION
end

desc 'Restore dependencies'
task :restore do
  sh "dotnet restore src/FubuLocalization.sln"
end

desc 'Run the unit tests'
task :test => [:compile] do
  sh "dotnet test src/FubuLocalization.sln"
end
...
desc 'Run the unit tests'
task :pack do
  sh "dotnet pack src/FubuLocalization/FubuLocalization.csproj -c #{COMPILE_TARGET} -o ./artifacts --no-build -p:PackageVersion=#{build_revision}"
end
